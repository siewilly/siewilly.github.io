...
## 套模板的過程
在決定要用Hexo當下我就已經找好要用的模板了，在茫茫模板中D-Sketon大大所製作的Reimu模板風格一下就對到了我（~~宅宅~~）的注意。但此時的我並不知道這才是地獄的開始....
...
接下來就是在將網站部署到GitHub Pages時，一直報錯`沒找到Reimu模板`，我就在這問題上卡了快一個月，直到寫這篇文的昨天，我在資料夾中加了一個名為.nojekyll的空文件，莫名其妙的地解決了這個問題。~~雖然我不完全理解為什麼這樣做有效，但至少讓我成功部署了。~~

## `Cousor AI`的影響
最近引起討論的`Vibe Coding`其核心理念是 **「完全順應感覺（vibes）、擁抱AI帶來的進步，並忘記程式碼本身的存在」** 雖然我這次寫網頁不太算`vibe coding` 但還是有近4成的問題是人工智慧幫我發現的，尤其是在模板修改和功能調整時。

但是，雖然LLM生成代碼為我帶來的便利性，但這也導致我對程式邏輯缺乏深入理解。如果生成的代碼出現問題，Debug過程可能變得困難，尤其是當問題隱藏在複雜邏輯中時，就會形成惡性循環。

...
接下來就是在將網站部署到GitHub Pages時，一直報錯`沒找到Reimu模板`，**這是因為github page預設就是用jekyll架構**，不過顯然的當時的我並不知道，我就在這問題上卡了快一個月，直到寫這篇文的前一天，我在資料夾中加了一個名為`.nojekyll`的空文件，莫名其妙的地解決了這個問題。~~雖然當時我不完全理解為什麼這樣做有效，但至少讓我成功部署了。~~
